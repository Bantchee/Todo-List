...
export const page = () => {
    ...
    projects.forEach((projectElement) => {
      projectElement.addEventListener("click", () => {
        ...
        for (let i = 0; i < directory.defaultProjects.length; i++) {
          if (
            directory.defaultProjects[i].name ==
        ...
        for (let i = 0; i < directory.createdProjects.length; i++) {
          if (
            directory.createdProjects[i].name ==
            projectElement.querySelector("p").textContent.toLowerCase()
          ) {
      ...
      detailsBtn.addEventListener("click", () => {
        ...
        if (detailsContainer) {
          ...
          detailsPara.textContent =
            directory.currentProject.tasks[index].details;
        }
      });
  ...
  const updateAddTaskInput = () => {
    ...
    addBtn.addEventListener("click", () => {
      ...
      const project = directory.getProject(
        projectValue.slice(0, 1).toLowerCase() + projectValue.slice(1)
      );
