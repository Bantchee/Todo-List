...
export const directory = (() => {
  const state = {
    defaultProjects: [project("inbox"), project("today"), project("upcoming")],
    createdProjects: [],
    currentProject: null,
  };
  ...
  const updateTodayTasks = () => {
    ...
    state.defaultProjects[0].tasks.forEach((task) => {
      if (getYear(currentDate) === getYear(task.createDueDate(task.dueDate))) {
        if (
          getMonth(currentDate) === getMonth(task.createDueDate(task.dueDate))
        ) {
          if (
            getDate(currentDate) === getDate(task.createDueDate(task.dueDate))
          ) {
    ...
    state.defaultProjects[0].tasks.forEach((task) => {
      if (getYear(pastDate) === getYear(task.createDueDate(task.dueDate))) {
        if (getMonth(pastDate) === getMonth(task.createDueDate(task.dueDate))) {
          if (
            getDate(pastDate) < getDate(task.createDueDate(task.dueDate)) &&
            getDate(task.createDueDate(task.dueDate)) < getDate(futureDate)
          ) {
